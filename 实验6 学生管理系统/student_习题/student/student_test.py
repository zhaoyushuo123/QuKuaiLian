...
try:
    ...
    student = student_contract.functions.select_id(min_age_id).call()
    student = tuple(student)
    add_point(10, "get mig_age_id(={}) student(={}) success!".format(min_age_id,student))
except Exception as e:
...
try:
        ...
        update_id = all_id2[0];
        update_dept = "dase";
        tx_hash = student_contract.functions.update_dept_by_id(update_id,update_dept).transact()
        tx_receipt = eth.waitForTransactionReceipt(tx_hash)

        student = student_contract.functions.select_id(update_id).call()
        student = tuple(student)
        print_info("student updated(={})".format(student))
        # 测试获取日志
        try:
            update_event = student_contract.events.Update.getLogs()[0]
            id = update_event.get('args').get('id')
            if update_id == id:
                add_point(10, "emit event success! event args: {}".format(update_event.get('args')))
            else:
                raise Exception
        except Exception as e:
            print_error("error: {}\nget event error! id = {}".format(e, update_id))
except Exception as e:
    print_error("error: {}\nupdate error! student: {}".format(e, student))
